...
export interface Treasury {
  ...
  label: string        // 'UST 4.25% Feb-36'
  cusip: string
  coupon: number
  maturity: string     // 'Feb-36'
  bidYield: number
  askYield: number
  bidPrice: number
  askPrice: number
  modDuration: number
  dv01PerMM: number    // DV01 per $1MM face, in $
  change: number       // yield change in bps from prior close
}
...
export interface CorpBond {
  ...
  bidPrice: number
  askPrice: number
  zSpread: number      // bps over interpolated treasury
  oas: number          // option-adjusted spread, bps
  modDuration: number
  dv01PerMM: number
  change: number       // yield change in bps
  isHY: boolean
}
...
export interface Position {
  ...
  faceValueMM: number   // $MM face (or # contracts for futures)
  avgPrice: number
  currentPrice: number
  currentYield: number
  marketValueMM: number
  modDuration: number
  dv01: number          // total DV01 for position in $
  cs01: number          // credit spread 01 (0 for rates)
  pnlToday: number      // in $
  pnlMtd: number
  pnlYtd: number
}
...
function perturbYield(y: number, level: number, slope: number, slopeFactor: number) {
  return Math.round((y + level + slope * slopeFactor) * 10000) / 10000
}

function yieldToPrice(prevYield: number, newYield: number, prevPrice: number, modDur: number) {
  const dY = (newYield - prevYield) / 100
  return Math.round((prevPrice - modDur * dY * prevPrice) * 10000) / 10000
}

export function useLiveMarketData() {
  const [treasuries, setTreasuries] = useState<Treasury[]>(BASE_TREASURIES)
  const [corpBonds, setCorpBonds] = useState<CorpBond[]>(BASE_CORP_BONDS)
  const [tFutures, setTFutures] = useState<TreasuryFuture[]>(BASE_TREASURY_FUTURES)
  const [sofrFutures, setSOFRFutures] = useState<SOFRFuture[]>(BASE_SOFR_FUTURES)
  const [cdxIndices, setCDXIndices] = useState<CDXIndex[]>(BASE_CDX)
  const [cdsNames, setCDSNames] = useState<CDS[]>(BASE_CDS)
  const [yieldCurve, setYieldCurve] = useState<YieldCurvePoint[]>(BASE_YIELD_CURVE)

  // slope factors: 2=short end, 1=belly, 0=long end
  const SLOPE_FACTORS = [2, 1.5, 1, 0.5, 0, -0.5, -0.5, -0.5, -0.5, 0, 0]

  const tick = () => {
    const level = (Math.random() - 0.5) * 0.015  // ±0.75bp level shift
    const slope = (Math.random() - 0.5) * 0.008  // ±0.4bp slope shift

    setTreasuries(prev => prev.map((t, i) => {
      const newBidYield = perturbYield(t.bidYield, level, slope, SLOPE_FACTORS[i] ?? 0)
      const newAskYield = perturbYield(t.askYield, level, slope, SLOPE_FACTORS[i] ?? 0)
    ...
    setSOFRFutures(prev => prev.map((f, i) => {
